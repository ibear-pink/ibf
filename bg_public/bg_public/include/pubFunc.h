...
//日期函数
long difftime(time_t startTime,time_t endTime,char * type);
time_t StringToDatetime(string str,int type);
...
char * Ltrim(char *str);
char * Rtrim(char * str);
string& sltrim(string &str);
string& srtrim(string &str);
...
int SplitString (const char *buff, char (*out_values)[64], int *size);
...
template<class K, class V> extern void clearDataList (vector<map<K, V*> > &mList);
...
template<typename K, typename V>
void clearDataList (vector<map<K, V*> > &mList)
{
	if (mList.size() == 0)
	{
		return;
	...
	map<K, V*> mMap;
	typename vector<map<K, V*> >::iterator iter;
	typename map<K, V*>::iterator vIter;
	for (iter = mList.begin(); iter != mList.end(); iter++)
	{
		mMap = *iter;
		for (vIter = mMap.begin(); vIter != mMap.end(); vIter++)
		{
			V *pData = vIter->second;
			delete pData;
			pData = NULL;
		}
		mMap.clear();
