...
int CDealDataMgr::Init(icfg_inifile *inicfg,int cur_processNum)
{
	...
	//加载ibear
	char className[64] = {0};
	strcpy(className,"myibear");
	Object* poObject = Object::CreateObject(className);
	m_Ibear = dynamic_cast<ibear*>(poObject);
	if (m_Inicfg->icfg_Key_GetValue("SYSTEM","db_num") == NULL)
	{
		//m_Ibear = new ibear(m_Inicfg->icfg_Key_GetValue("SYSTEM","ibear_name"),NULL);
		m_Ibear->initbear(m_Inicfg->icfg_Key_GetValue("SYSTEM","ibear_name"),NULL,NULL);
	...
	else
	{
		...
		m_Ibear->initbear(m_Inicfg->icfg_Key_GetValue("SYSTEM","ibear_name"),pInfo,NULL);
	}
	...
	strcpy(im_intertype->chName,"interface_type");
	...
	if (m_Inicfg->icfg_Key_GetValue("ZK_SERVER","FLB_NAME") != NULL)
	{
		char flb_name[128] = {0};
		strcpy(flb_name,m_Inicfg->icfg_Key_GetValue("ZK_SERVER","FLB_NAME"));
		strreplace(m_Indir,m_Indir,(char *)FLB_NAME,flb_name);
		strreplace(m_Bakdir,m_Bakdir,(char *)FLB_NAME,flb_name);
		strreplace(m_Errdir,m_Errdir,(char *)FLB_NAME,flb_name);
		strreplace(m_Breakdir,m_Breakdir,(char *)FLB_NAME,flb_name);
	}
...
int CDealDataMgr::DealFileList()
{
	...
	for (vector<stFileInfo *>::iterator it = file->mFileList.begin();it!=file->mFileList.end();it++)
	{
		stFileInfo *sf = *it;
#ifdef _DEBUG_INFO_
		printf("[%d]%s,%s\n",m_InterfaceType,sf->sFileName,sf->sModifyTime);
#endif
		//开始处理
		switch (m_InterfaceType)
		{
...
int CDealDataMgr::dealFile(char *filename)
{
	...
	while(!file->IsFileEnd())
	{
		...
		{
			pTask->code = m_PublicMgr->deal_process(pTask);
		}
	}
	delete file;
	file = NULL;
			...
			case INTERFACE_TYPE_TABLE:
			{
				...
				if (m_GiveType == 1)
				{
					for (vector<stDealTask*>::iterator dataIter = m_interfaceTable.begin(); dataIter != m_interfaceTable.end(); dataIter++)
					{
						...
						//将池中数据修改标示
						if (m_PublicMgr->UpdPoorStatus(pTask) != 0)
						{
							setThreadState(THREAD_STAT_SUSPEND);
						}
						putnum ++;
						if (putnum/100 == 0)
						{
				...
				switch(m_TabType)
				{
					case BACKUP_TYPE_FILE:
					{
						for (vector<stDealTask*>::iterator iter = m_interfaceTable.begin(); iter != m_interfaceTable.end(); iter++)
						{
							...
							strcpy(tFileInfo->fileNameHead,dt->type);
							sprintf(tFileInfo->fileName,"%s_%d",tFileInfo->fileNameHead,m_ProcessNum);
							sprintf(tFileInfo->fileContent,"%s|%s\n",dt->dataSeq,(char*)dt->sBuff);
