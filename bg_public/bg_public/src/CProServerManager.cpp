...
bool CProServerManager::Start (map<string,string> classMap)
{
	...
	for (int i = 0;i < m_CfgInfo->log_file_num ;i++)
	{
		...
		sprintf(chname, "log_invalid%d",i);
		if (strcmp(m_iniCfg->icfg_Key_GetValue("LOG",chname,"false"),"false") == 0)
		{
			m_CfgInfo->logfile_infos[i].log_invalid = false;
		}
		else
		{
			m_CfgInfo->logfile_infos[i].log_invalid = true;
		}
	...
	char flb_name[64] = {0};
	SplitAtPos(zkCfgInfo->sInBuf,flb_name,';',3);
	printf("flb_name1=%s\n",flb_name);
	m_iniCfg->icfg_SetKey((char *)"ZK_SERVER",(char *)"FLB_NAME",flb_name);
