...
int Check_BDSData::process(stDealTask *pTask)
{
	...
	printf("[%d]Check_BDSData::process[%s][%s]\n",m_CurThreadNum,m_BusinessType,pTask->dataSeq);
	//WriteLog(m_LogName, LOG_SYS_RUNNING,m_CurThreadNum, "[%d]Check_BDSData::process[%s]\n",m_CurThreadNum,(char*)pTask->sBuff);
	return iRet;
}

int Check_BDSData::fprocess(stDealTask *pTask)
{
	int iRet = 0;
	printf("[%d]Check_BDSData::fprocess[%s][%s]\n",m_CurThreadNum,m_BusinessType,(char*)pTask->sBuff);
	WriteLog(m_LogName, LOG_SYS_RUNNING,m_CurThreadNum, "[%d]Check_BDSData::fprocess[%s]\n",m_CurThreadNum,(char*)pTask->sBuff);
	return iRet;
}
