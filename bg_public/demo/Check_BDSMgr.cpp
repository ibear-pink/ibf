...
int Check_BDSMgr::init(ibear *br, char *logName,int thread_num,map<string,icfg_Map*> user_defined)
{
	...
	map<string,icfg_Map*>::iterator it_cur = user_defined.find(string("CUR_NUM"));
	if (it_cur!=user_defined.end())
	{
		m_CurNum = atoi(it_cur->second->chValue);
	}
	return 0;
}
...
int Check_BDSMgr::deal_process(stDealTask *pTask)
{
	printf("Check_BDSMgr->deal_process[%s]\n",pTask->sBuff);
	return 0;
}
...
int Check_BDSMgr::ProInterface(vector<stDealTask*> *values)
{
	...
	map<string,string> inParams;
	vector<vector<string> > list;
	inParams.insert(pair<string, string>(string("CUR_NUM"),toString(m_CurNum)));
