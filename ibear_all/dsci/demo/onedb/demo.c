...
int main()
{
    ...
    /*连接初始化*/
    pD = &myDSC;
    if(0 != DSCI_init_direct(errStr, pD, "predeal/predeal@oracle:billdb"))
    {
        printf("init %s\n", errStr);
        return 0;
    }   
    ...
    DSCI_bind_param(pD, &ss1, 1, DSCI_TYPE_DOUBLE, &bb);
    DSCI_bind_param(pD, &ss1, 2, DSCI_TYPE_DATE, str+200);
